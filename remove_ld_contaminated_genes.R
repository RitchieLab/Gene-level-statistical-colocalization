...
opt_list <- list(
  ...
  make_option("--coloc_p_h3_threshold",type="numeric",default=0,5,help="Threshold for minP[H3] peer gene across all lead SNPs in the gene"),
  make_option("--coloc_p_h4_threshold",type="numeric",default=0.5,help="Threshold for maxP[H4] per gene across all lead SNPs in the gene"),
...
if(length(gene.list)==0) stop("No genes that pass the LD contamination filter") else write.table(paste0(output.folder,"/Final_list_of_genes_no_ld_contamination.txt"),row.names=F,col.names=F,sep="\t",quote=F)
