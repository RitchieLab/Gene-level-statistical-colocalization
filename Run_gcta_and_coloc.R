...
opt_list <- list(
  ...
  make_option("--tissue",type="character",help="Tissuee name"),
  make_option("--gwas_data_name",type="character",help="Name of GWAS gwas.data.name (e.g. GLGC, GIANT)"),
...
#4.2 If there are at least 2 independent SNPs  (other than the lead SNP) that are greater than 100KB from the lead SNP, run GCTA to perform association analysis of the included SNPs conditional on the given list of independent SNPs. Results are saved in the .cma.cojo file
     ...
     gwas.cond[,"p"] <- gwas.cond[,"pC"]
       } else {gwas.cond <- x; colnames(gwas.cond) <- c("Chr","bp","SNP","refA","A2","freq","b","se","p","n")}
      } else {gwas.cond <- x; colnames(gwas.cond) <- c("Chr","bp","SNP","refA","A2","freq","b","se","p","n")}
...
#4.3 Repeat steps 4.1 and 4.2 for all "eGenes" to get conditionally independent SNPs for the eQTL dataset instead of GWAS dataset
    ...
    for(k in 1:length(genes_fin)){
        ...
        x2_gcta <- x2[,c("SNP","A1","A2","maf","slope","slope_se","pval_nominal")]
        x2_gcta$N <- N.eqtl.tissue
        ...
        if(nrow(eqtl.topsnps)>=2) {
          ...
          eqtl.cond <- x; colnames(eqtl.cond) = c("Chr","bp","SNP","refA","A2","freq","b","se","p","n")
        }
        eqtl.topsnps <- read.table(paste0(tissue,"_chr_",chr,"_pos_",pos,"_",genes_fin[k],".jma.cojo"),header=T)
        ...
        eqtl.maf <- eqtl.cond2[,"freq_geno"]
        gwas.maf <- ifelse(gwas.cond2[,"freq_geno"]<0.5,gwas.cond2[,"freq_geno"],1-gwas.cond2[,"freq_geno"])
...
#5.2 Prepare output dataset 
           ...
           coloc_probs[,"GWAS.P"] <- x[which(x[,"SNP"]==rsid),"P"][1]
           coloc_probs[,"GWAS.Beta"] <- x[which(x[,"SNP"]==rsid),"BETA"][1]
           coloc_probs[,"GWAS.SE"] <- x[which(x[,"SNP"]==rsid),"SE"][1]
           coloc_probs[,"GWAS.Data"] <- gwas.data.name
           coloc_probs[,"GWAS.A1"] <- x[which(x[,"SNP"]==rsid),"A1"][1]
           coloc_probs[,"GWAS.A2"] <- x[which(x[,"SNP"]==rsid),"A2"][1]
           coloc_probs[,"GWAS.Freq"] <- x[which(x[,"SNP"]==rsid),"FREQ"][1]
           ...
           coloc_probs[,"eQTL.Beta"] <- x2[which(x2[,"SNP"]==rsid),"slope"][1]
           coloc_probs[,"eQTL.SE"] <- x2[which(x2[,"SNP"]==rsid),"slope_se"][1]
           coloc_probs[,"eQTL.P"] <- x2[which(x2[,"SNP"]==rsid),"pval_nominal"][1]
           coloc_probs[,"eQTL.A1"] <- x2[which(x2[,"SNP"]==rsid),"A1"][1]
           coloc_probs[,"eQTL.A2"] <- x2[which(x2[,"SNP"]==rsid),"A2"][1]
           ...
           coloc_probs[,"GWAS.N"] <-  x[which(paste0(x[,"CHR"],":",x[,"BP"])==rsid),"N"][1]
           coloc_probs[,"eQTL.N"] <-  x2[which(paste0(x2[,"CHR"],":",x[,"BP"])==rsid),"N"][1]
           ...
           message("tissue=",tissue," / j=",j," / k=",k," / num.genes=", length(genes_fin), " / ", trait," / ",gwas.data.name," / num.lead.snps=",length(lead_snps))
